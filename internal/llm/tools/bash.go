...
var bannedCommands = []string{
	"alias", "curlie", "axel", "aria2c",
	"lynx", "w3m", "links", "httpie", "xh",
	"http-prompt", "chrome", "safari",
}
...
var dangerousCommands = []string{
	"rm", "rmdir", "cp", "mv", "chmod", "chgrp", "chown", "curl", "nc", "telnet", "wget",
	"sudo", "git push", "git reset --hard", "git clean", "git rebase", "firefox",
...
// without confirmation when they only touch files under /tmp (e.g. testing).
var tmpExemptCommands = []string{
...
🤖 Generated with hylbscode
Co-Authored-By: hylbscode <noreply@hylbscode.ai>
...
func (b *bashTool) Run(ctx context.Context, call ToolCall) (ToolResponse, error) {
	...
	streamCallback := GetStreamCallback(ctx)
	stdout, stderr, exitCode, interrupted, err := shell.ExecStream(ctx, params.Command, params.Timeout, func(chunk string) {
...
func isDangerousCommand(cmdLower, binary string) bool {
	...
	for _, sep := range []string{"&&", "||", ";", "|"} {
		if strings.Contains(cmdLower, sep+binary) {
			return true
		}
	}
	return false
...
// files under /tmp, in which case it can run without confirmation.
func isTmpExempt(cmdLower, binary string) bool {
	for _, b := range tmpExemptCommands {
		if b != binary {
			continue
		}
		// Follow the command invocation (first word or after a separator).
		body := cmdLower
		for _, sep := range []string{"&&", "||", ";", "|"} {
			if idx := strings.LastIndex(body, sep); idx >= 0 {
				body = body[idx+len(sep):]
				break
			}
		}
		// Extract arguments after the command word.
