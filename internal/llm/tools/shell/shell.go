...
// outputThrottle is the minimum interval between streaming output callbacks.
const outputThrottle = 300 * time.Millisecond
...
func (s *PersistentShell) processCommands() {
	for cmd := range s.commandQueue {
		result := s.execCommand(cmd.command, cmd.timeout, cmd.ctx, cmd.onOutput)
		cmd.resultChan <- result
	}
}

func (s *PersistentShell) execCommand(command string, timeout time.Duration, ctx context.Context, onOutput func(string)) commandResult {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	tempDir := os.TempDir()
	stdoutFile := filepath.Join(tempDir, fmt.Sprintf("hylbscode-stdout-%d", time.Now().UnixNano()))
	stderrFile := filepath.Join(tempDir, fmt.Sprintf("hylbscode-stderr-%d", time.Now().UnixNano()))
	statusFile := filepath.Join(tempDir, fmt.Sprintf("hylbscode-status-%d", time.Now().UnixNano()))
	cwdFile := filepath.Join(tempDir, fmt.Sprintf("hylbscode-cwd-%d", time.Now().UnixNano()))
	...
	go func() {
		...
		for {
			...
			case <-time.After(10 * time.Millisecond):
				// Stream partial stdout to the callback roughly every 300ms.
				if onOutput != nil && time.Since(lastOutput) >= outputThrottle {
					cur := readFileOrEmpty(stdoutFile)
					curTrimmed := strings.TrimSuffix(cur, "\n")
					if len(curTrimmed) > lastLen {
						newChunk := curTrimmed[lastLen:]
						lastLen = len(curTrimmed)
						lastOutput = time.Now()
						onOutput(newChunk)
					}
				}
...
func (s *PersistentShell) Exec(ctx context.Context, command string, timeoutMs int) (string, string, int, bool, error) {
	return s.ExecStream(ctx, command, timeoutMs, nil)
}

// ExecStream runs the command and, while it executes, delivers partial stdout
// to onOutput roughly every outputThrottle. The final returned stdout contains
// the complete output.
func (s *PersistentShell) ExecStream(ctx context.Context, command string, timeoutMs int, onOutput func(string)) (string, string, int, bool, error) {
	if !s.isAlive {
