...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	q := Queries{db: db}
	var err error
	if q.bulkDeleteTodosBySessionStmt, err = db.PrepareContext(ctx, bulkDeleteTodosBySession); err != nil {
		return nil, fmt.Errorf("error preparing query BulkDeleteTodosBySession: %w", err)
	}
...
func (q *Queries) Close() error {
	var err error
	if q.bulkDeleteTodosBySessionStmt != nil {
		if cerr := q.bulkDeleteTodosBySessionStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing bulkDeleteTodosBySessionStmt: %w", cerr)
		}
	}
...
type Queries struct {
	db                           DBTX
	tx                           *sql.Tx
	bulkDeleteTodosBySessionStmt *sql.Stmt
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		db:                           tx,
		tx:                           tx,
		bulkDeleteTodosBySessionStmt: q.bulkDeleteTodosBySessionStmt,
