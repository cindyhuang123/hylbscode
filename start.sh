#!/usr/bin/env bash
# 启动 hylbscode, 按发行版处理中文输入法(XIM):
#   - Fedora: fcitx5 体系。启动前判断式修复 fcitx5 XIM 注册, 并以正确环境变量启动。
#   - 其他发行版(如 Ubuntu): 使用桌面会话自带的输入法(ibus 等), 不做任何干预,
#     直接透传会话环境变量启动 hylbscode。
set -euo pipefail
